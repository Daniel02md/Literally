...
//                            .multilineTextAlignment(.center)
                        ...
                        VStack(){
                            ...
                            VStack(){
                                ForEach(recomendations, id: \.self) { book in
                                    let img: URL = URL(string: "\(book.coverImageURL)")!

                                    
                                    NavigationLink(destination:{
                                        MovieView(imageUrl: img, title: book.title, rate: book.rate, description: "\(book.description)")
                                    }){
                                        ItemListView(imageUrl: img, title: "\(book.title)", author: "\(book.authors[0])", rate: book.rate)
    ...
    struct MoviesView_Previews: PreviewProvider {
        static var previews: some View {
            ...
            MovieView (
                imageUrl: img, title: "Jojo Rabbit", rate: 5, description: "Jojo é um garoto alemão solitário que descobre que sua mãe está escondendo uma garota judia no sótão. Ajudado apenas por seu amigo imaginário, Adolf Hitler, Jojo deve enfrentar seu nacionalismo cego enquanto a Segunda Guerra Mundial prossegue.")
